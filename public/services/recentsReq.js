function recentsRequest(limit) {
  ...
  const isYesterday = (someDate) => {
    let now = new Date();
    return someDate.getDate() == now.getDate() - 1
        && someDate.getMonth() == now.getMonth()
        && someDate.getFullYear() == now.getFullYear();
  };
  ...
  function formatDate(iso) {
    ...
    if (isToday(date)) {
      ...
      return '' + ret + format_two_digits(date.getHours()) + ':' + format_two_digits(date.getMinutes());
    }

    if (isYesterday(date)) {
      return 'yesterday ' + format_two_digits(date.getHours()) + ':' + format_two_digits(date.getMinutes());
    }
